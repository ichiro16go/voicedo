...
| 録音 | **expo-av**（iOS/Android/Web 統一API） | バックグラウンド録音もネイティブで取れる |
...
| API層 | **Next.js (Vercel) on API routes** or **Supabase Edge Functions** | LLM呼び出しはサーバー側 |
...
- [ ] expo-av で iOS/Android/Web 録音の音質・サイズ・コーデック差確認
...
3. expo-av での録音 → アップロード → STT → 質問LLM → 記事生成パイプライン
...
### 3. 対話LLM → **gpt-5-mini（メイン） + Gemini 2.5 Flash（フォールバック）**

**決定理由**
- 「ハル」は短い質問を高速に出すのが仕事 → コスト/速度重視。
- gpt-5-mini は Function Calling / Structured Output が安定。質問テンプレ5パターンの切替えに便利。
- マルチプロバイダー抽象化（plan.md リスク⑩対策）を **最初から** Vercel AI SDK で実装。

**人格切替**
- ハル（雑記/note）: system prompt A + temperature 0.7
- ロイ（技術/Qiita）: system prompt B + temperature 0.4

---

### 4. 記事生成LLM → **Claude Sonnet 4.5**

**決定理由**
- Voicedo の差別化の核は「生成された記事の品質」。ここはケチらない。
- 日本語の文章のリズム・敬体/常体の一貫性は Claude が現状最強（社内検証多数）。
- 1セッション1回のみの呼び出しなので、原価インパクトは限定的（$0.03/セッション）。
- Anthropic は **デフォルトで API入力をモデル学習に使わない**（plan.md プライバシー要件と整合）。
...
### 6. 決済 → **Stripe（Subscriptions + One-time + Deposits）**

**決定理由**
- plan.md の Stage 0 課金検証「¥500記事 / ¥1,980伴走 / ¥500デポジット / Standard ¥980サブスク」を **すべてStripeでカバー可能**。
- Stripe Customer Portal で解約・プラン変更を顧客側に委任（運用負荷ゼロ）
- 日本の決済手数料 3.6%、サブスクの月次自動課金、領収書発行まで標準装備

**実装順序**
1. Stage 0：Payment Links（コード不要、URL貼るだけ）で実決済を集める
2. Stage 1：Stripe Checkout を組み込む
3. Stage 2：Customer Portal + Subscription + Webhook ハンドリング
...
- **pnpm**：依存解決が速い、Vercel と相性良
- **Biome**：ESLint + Prettier 統合、起動と整形が圧倒的に速い（Ichiro 一人体制で価値大）
- **Vitest**：ユニット・統合
- **Playwright**：noteへの貼り付けフロー（Plan A/B/C）の実機検証を自動化候補
- **Storybook は当面入れない**：Stage 2 までは shadcn/ui のドキュメントで十分

## CI/CD
- GitHub Actions：lint / test / typecheck / preview deploy
- Vercel Preview：PR毎に自動デプロイ、Ichiro 自身のスマホで即確認
- main マージで本番デプロイ
...
- ❌ ネイティブアプリ
- ❌ オフライン録音 / バックグラウンド録音
- ❌ 独自Auth（Supabase Auth + Google/Magic Link で十分）
- ❌ Kubernetes / 独自インフラ
- ❌ GraphQL（Supabase REST/RPC で十分）
- ❌ 多言語化（日本語のみ、最初の半年は🅰フォーカス）
- ❌ ソーシャル機能（フォロー・いいね、長期戦略の番組モードで再検討）
...
- [ ] iOS Safari 17+ で MediaRecorder + audio/mp4 が動くか
- [ ] iOS PWA で Web Push が朝7時に届くか
- [ ] note貼り付けUX 3パターン実機検証（plan.md プラットフォーム連携セクション）
- [ ] gpt-4o-transcribe で5分音声の精度・レイテンシ実測
- [ ] Claude Sonnet 4.5 で実音声→記事生成の品質確認（5サンプル）
- [ ] Supabase RLS で「他人の音声を取れない」を E2E テスト
- [ ] Stripe Payment Links で ¥500 実決済を Ichiro 自身がテスト
...
1. `voicedo` リポジトリ初期化（Next.js 15 + TypeScript + Tailwind + shadcn/ui）
2. Supabase プロジェクト作成、スキーマ初版適用
3. `/record` 画面（MediaRecorder + アップロード）
4. STT → 質問LLM → 記事生成のパイプライン構築
5. Stripe Payment Links で Stage 0 課金導線を先に立てる
6. note貼り付けUX Plan A/B/C を半日で実機検証

---

## 改訂履歴
- 2026-06-22 v1.0：初版作成
